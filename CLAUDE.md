...
- ⚠️ **Первый боевой прогон (run #13) отменён по таймауту (45 мин).** Фид `yandex.ru/adv/news/rss` завис на скачивании ~38 мин: `feedparser.parse(url)` **не держит сетевой таймаут**, зависший хост подвесил весь `collect`. Плюс за прогон собралось **1620 items** (обычно ~100) — архивный хвост фидов. **Фикс в коллекторе (`rss_collector.py`):** (1) скачивание фида через `requests` с таймаутом `FEED_FETCH_TIMEOUT=20` → зависший хост скипается; (2) **фильтр свежести `MAX_AGE_DAYS=30`** — на первичном скрининге берём только за последний месяц (режет 1620 → ~сотня); (3) кап `MAX_ENTRIES_PER_FEED=120`; (4) лог `+N свежих` по каждому фиду. Тесты: `tests/test_collector.py`.
